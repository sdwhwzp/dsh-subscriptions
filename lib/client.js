  ...
  factory: (require) => {
    ...
    class ErrorBoundary extends React.Component {
      ...
      componentDidCatch(error, info) {
        try { console.error("[dsh-subscriptions UI error]", error, info) } catch (err) { /* fallback if console fails */ }
      }
      render() {
    ...
    '.dsub-btnActive{background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-text-contrast,#fff);border-color:var(--dsw-alias-brand-primary)}' +
    ...
    '.dsub-cooldown{display:inline-block;padding:2px 6px;border-radius:4px;background:var(--dsw-alias-state-warning-primary);color:var(--dsw-alias-bg-layer-1,#000);font-size:11px;font-weight:600}' +
    ...
    '.dsub-brandGrok{background:color-mix(in srgb,#a855f7 15%,transparent);color:#c084fc;border:1px solid color-mix(in srgb,#a855f7 30%,transparent)}' +
    ...
    '.dsub-brandKimi{background:color-mix(in srgb,#ec4899 15%,transparent);color:#f472b6;border:1px solid color-mix(in srgb,#ec4899 30%,transparent)}' +
    '.dsub-brandGlm{background:color-mix(in srgb,#14b8a6 15%,transparent);color:#2dd4bf;border:1px solid color-mix(in srgb,#14b8a6 30%,transparent)}' +
    ...
    '.dsub-btnPrimary{display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border-radius:8px;font-size:12px;font-weight:600;background:var(--dsw-alias-brand-primary,#3b82f6);color:var(--dsw-alias-text-contrast,#fff);border:0;cursor:pointer;transition:opacity .15s}' +
...
const cssId = 'dsh-subscriptions/settings.module.css'
    ...
    function badgeFor(account, t) {
      ...
      // #303: a quarantined account says why it is parked.
    ...
    function SubsSection(props) {
      ...
      const genDiag = () => {
        fetch('/dsh-subscriptions/diagnostics', { cache: 'no-store' })
          ...
          .then((d) => {
            ...
            setDiag(txt)
            try { navigator.clipboard.writeText(txt) } catch (err) { /* clipboard write disallowed */ }
          })
      ...
      const checkUpdater = () => {
        ...
        return fetch('/dsh-subscriptions/update', { cache: 'no-store' })
          .then((res) => res.json())
          .then((data) => {
      ...
      const performUpdate = async () => {
        ...
        try {
          const res = await fetch('/dsh-subscriptions/update', {
            method: 'POST',
            headers: {
      ...
      const reloadTelemetry = () => {
        fetch('/dsh-subscriptions/telemetry', { cache: 'no-store' })
          .then((r) => r.json())
      ...
      const runSmokeTest = async (provider, index) => {
        ...
        try {
          const res = await fetch('/dsh-subscriptions/smoke', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ provider, index }),
          })
      ...
      const reload = () => {
        const t0 = Date.now()
        return fetch('/dsh-subscriptions/config', { cache: 'no-store' })
          .then((res) => res.json())
          .then((data) => {
      ...
      return React.createElement('div', { className: 'dsub-wrap' },
          ...
          React.createElement('div', { style: { padding: '8px 0', overflowX: 'auto' } },
            React.createElement('table', { style: { width: '100%', fontSize: '12px', borderCollapse: 'collapse' } },
              React.createElement('thead', null,
                React.createElement('tr', { style: { borderBottom: '1px solid var(--dsw-alias-border-primary, rgba(255,255,255,0.1))', textAlign: 'left', opacity: 0.7 } },
              ...
              React.createElement('tbody', null,
                accounts.map((acc, idx) => {
                  ...
                  const healthColor = sc >= 80 ? '#10b981' : (sc >= 50 ? '#f59e0b' : '#ef4444')
                  ...
                  return React.createElement('tr', { key: (acc.ref || acc.provider + acc.index || idx), style: { borderBottom: '1px solid var(--dsw-alias-border-secondary, rgba(255,255,255,0.05))' } },
        ...
        React.createElement('div', { className: 'dsub-block' },
          ...
          diag ? React.createElement('div', { className: 'dsub-row' },
            React.createElement('button', {
              type: 'button', className: 'dsub-mini',
              onClick: () => { try { navigator.clipboard.writeText(diag) } catch (err) { /* clipboard write error */ } },
            }, t('diagCopy')),
                  ...
                  return React.createElement('div', { className: 'dsub-verify' },
                    ...
                    d.status === 'pending' ? React.createElement(React.Fragment, null,
                      ...
                      React.createElement('div', { className: 'dsub-row' },
                        React.createElement('button', {
                          type: 'button', className: 'dsub-mini',
                          onClick: () => { try { navigator.clipboard.writeText(d.userCode) } catch (err) { /* clipboard write error */ } },
                        }, t('deviceCopy')),
