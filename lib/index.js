...
import { resolveFallbackVendor, mapFallbackModel, DEFAULT_CASCADE_CHAINS } from './cascade.js'
import { exportVault, importVault } from './vault.js'
import { recordAlert, getRecentAlerts, checkQuotaThresholds, notifySessionExpired } from './alerts.js'
import { calculateBurnRatePerHour, computePacingRisk } from './forecast.js'
import { needsWarmupProbe, enterProbing, resolveWarmupSuccess, resolveWarmupFailure } from './quarantine.js'
...
export function apply(ctx, config) {
  ...
  let settingsApi
  // #367: maintain a single immutable config snapshot without structuredClone on hot path
  let cachedConfig = Config(config ?? {}) ?? config

  function syncSnapshot(raw) {
    if (raw !== undefined && raw !== null) {
      try {
        cachedConfig = Config(raw ?? {}) ?? config
      } catch {
        cachedConfig = config
      }
    } else if (settingsApi && typeof settingsApi.get === 'function') {
      try {
        const rawSettings = settingsApi.get()
        cachedConfig = Config(rawSettings ?? {}) ?? config
      } catch {
        cachedConfig = config
      }
    }
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    sctx.effect(() => () => {
      ...
      cachedConfig = Config(config ?? {}) ?? config
      syncCustomVendors()
    })
  })
  ...
  async function* cascadeFallbackStream({ options, provider, err, visited = new Set() }) {
    ...
    } catch (e) { /* ignore cascade history recording error */ }
  }
